...
void * funct_r_processaudio (void * c_globaldatain ) {
...
// thisformat: 1: dvtool-file format, 2: DSTAR stream, 3: raw
...
if ((p_r_global->rawsyncsize < 1) || (p_r_global->rawsyncsize > 16)) {
	fprintf(stderr,"Error in processaudio, rawsynsize should be between 1 and 16, got %d. Exiting!\n",p_r_global->rawsyncsize);
	exit(-1);
}; // end if
rawsyncmask=size2mask[p_r_global->rawsyncsize-1];
...
while (!(thisfileend)) {
	...
	}; // end if (state == 3) && (maxaudiolevelvalid)
	...
	for (sampleloop=0; sampleloop <thisbuffersize; sampleloop++) {
		...
		if (state == 0) {
			...
			// start looking for frame sync if we have received sufficient bitsync
				...
				if (syncreceived > 20) {
					syncAA=0;
					bitmatch=countdiff16(last2octets,rawsyncmask,p_r_global->rawsyncsize,
						p_r_global->rawsync,BITERRORSSTARTSYN);
				} else {
					syncAA=1;
					bitmatch=countdiff16(last2octets,rawsyncmask,p_r_global->rawsyncsize,
						p_r_global->rawsync,0);
				}; // end if
				...
				if (bitmatch) {
					int p;
					// OK, we have a valid frame sync, go to state 1 (dvtool file-format or D-STAR UDP stream) or 3 (raw)
					...
					// if raw, open Destination Abstraction Layer and WRITE
					if (thisformat == 3) {
						...
						if (p_r_global->rawsyncsize & 0x01) {
							// odd
							if (!p_g_global->rawinvert) {
								// invert bit order
							...
							mask=0x5555 & ~size2mask[(p_r_global->rawsyncsize) -1];
						} else {
							// even
							if (!p_g_global->rawinvert) {
								// invert bit order
							...
							// do not invert mask (will be done later)
							mask=0xaaaa & ~size2mask[(p_r_global->rawsyncsize) -1];
						}; // end else - if (odd or even)

						// create last 2 octets, always non-inverted at this stage
						syncpattern=p_r_global->rawsync & size2mask[(p_r_global->rawsyncsize) -1];
						syncpattern |= mask;
