...
if (cmd == IDAL_CMD_OPEN) {
	...
	// output methode 2: file
		...
		} else if (p_r_global->recformat == 3) {
			snprintf(fnamefull,160,"%s-%04d.dstrraw",p_r_global->fnameout,filecount);
		} else {
	...
	if (p_r_global->udpport) {
		int ret;
		struct addrinfo hint;
		struct addrinfo *info;


		if ((p_r_global->udpport < 0) || (p_r_global->udpport > 65535)) {
		...
		udpsockaddr->sin6_port=htons((unsigned short int) r_global.udpport);
