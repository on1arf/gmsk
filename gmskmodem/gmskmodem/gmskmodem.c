...
// At the same time, read a dvtool or raw binary file and stream it out
...
if (!s_global.disable) {
	...
	} else if (g_global.format == 3) {
		pthread_create(&thr_s_input, NULL, funct_input_raw, (void *) &c_global); 
	} else {
		fprintf(stderr,"error sender unsupported format %d!\n",g_global.format);
		exit(-1);
...
if (!r_global.disable) {
	...
	pthread_create(&thr_r_processaudio, NULL, funct_r_processaudio, (void *) &c_global);
