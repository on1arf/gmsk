...
//	int audioread; // audioread removed as r_capture is now thread instead
						// of timed interrupt
	char * fnameout;
	int outtostdout;
	char * udpout_host;
	...
	int rawsyncsize;
	int sendmarker;
	int recformat; // 0=undef, 1=dvtool, 2=dstar stream, 3=raw
	int disable;
} r_globaldatastr;
...
typedef struct {
	...
	int format; // 0=undef, 1=dvtool, 2=dstar stream, 3=raw
	int rawinvert;
...
unsigned char startsync_pattern[]= {0x55, 0x55, 0x55, 0x55, 0x55, 0x55,
