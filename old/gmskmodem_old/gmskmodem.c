...
#include "dspstuff.h"
//#include "descramble.h" // tempory removed untill header processing is added
//#include "fcs.h" // tempory remove until header processing is added
...
if (!s_global.disable) {
	...
	pthread_create(&thr_s_gmskmodulate, NULL, funct_s_modulateandbuffer, (void *) &s_global);
		...
		} else if (s_global.ptt_gpiopin >= 0) {
			pthread_create(&thr_s_ptt, NULL, funct_pttgpio, (void *) &c_global);
		}; // end elsif - elsif - if
	#endif
