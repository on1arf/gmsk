...
void * funct_processaudio_int (void * globaldatain ) {
...
while (!(thisfileend)) {
	...
	nextbuffer=(p_global->pntr_int_process + 1) & buffermask;

	if (p_global->pntr_int_capture == nextbuffer) {
	...
	audiobuffer= (int16_t *) p_global->int_buffer[thisbuffer];
	thisbuffersize=p_global->int_buffersize[thisbuffer];

	if (global.fileorcapture) {
		thisfileend=p_global->int_fileend[thisbuffer];
	}; // end if

	if (global.dumpaverage) {
		printf("(%04X)",p_global->int_audioaverage[thisbuffer]);
	}; // end if
	...
	for (sampleloop=0; sampleloop <thisbuffersize; sampleloop++) {
		...
		if (state == 1) {
			...
			if (radioheaderreceived >= 660) {
				...
				snprintf(fnamefull,160,"%s-%04d-int.dvtool",p_global->fnameout,filecount);
				...
				// init vars
				// streamid is supposted to be a 2 byte random number
				// we set the streamid fixed to "0x1234" as it our goal to compair the two files 
				streamid=0x1234;
...
//				}; // end else - if
	...
	p_global->pntr_int_process = nextbuffer;
...
void * funct_processaudio_float (void * globaldatain ) {
...
while (!(thisfileend)) {
	...
	// check the next buffer slot. If the "capture" process is using it, we do not have any
	// audio available. -> wait and try again
	nextbuffer=(p_global->pntr_float_process + 1) & buffermask;

	if (p_global->pntr_float_capture == nextbuffer) {
		// nothing to process: sleep for 1/4 ms
		usleep((useconds_t) 250);
		continue;
	};
	...
	// per sample
	thisbuffer=nextbuffer;
	audiobuffer= (int16_t *) p_global->float_buffer[thisbuffer];
	thisbuffersize=p_global->float_buffersize[thisbuffer];

	if (global.fileorcapture) {
		thisfileend=p_global->float_fileend[thisbuffer];
	}; // end if

	if (global.dumpaverage) {
		printf("(%04X)",p_global->float_audioaverage[thisbuffer]);
	}; // end if
	...
	for (sampleloop=0; sampleloop <thisbuffersize; sampleloop++) {
		...
		if (state == 1) {
			...
			if (radioheaderreceived >= 660) {
				...
				// open file for output
				snprintf(fnamefull,160,"%s-%04d-float.dvtool",p_global->fnameout,filecount);
...
//				}; // end else - if
	...
	p_global->pntr_float_process = nextbuffer;
