...
// added by Kr. Bonne for uint32_t
#include <stdint.h>
...
/*---------------------------------------------------------------------------*\
  ...
  Given 12 bits of data reiurns a 23 bit codeword for transmission
  over the channel.

Slighly modified by Kristoff Bonne (ON1ARF) to better suite the c2gmskAPI
\*---------------------------------------------------------------------------*/

int golay23_encode(unsigned char *p_in) {
	// Kr. Bonne (4/june/2013):
	// p_in is pointer to unsigned char array of at least 3 elements
	// first 12 bits are input
	// next 12 bits are output
	// bits are ordered lsb first: 01234567 0123xxxx xxxxxx
	// always returns 0
...
//    assert(inited);
	...
	// copy octet per octet to be independant of endianess
	in=(((int) p_in[0])) + ((((int) p_in[1]) & 0x0f) << 8);

//    assert(data <= 0xfff);
	encoded=encoding_table[in];

	p_in[0]=(unsigned char) ((encoded)&0xff);
	p_in[1]=(unsigned char) ((encoded>>8)&0xff);
	p_in[2]=(unsigned char) ((encoded>>16)&0xff);
...
int golay23_decode(unsigned char * p_in) {
	...
	// decoded data is written back in first 12 bits

	// return number of bit errors

	uint32_t in, decoded;
	int errors;
	unsigned int diff; // make unsigned to force logical shift right 
