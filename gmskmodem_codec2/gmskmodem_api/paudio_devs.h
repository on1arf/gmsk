...
// check audio device 
int checkaudiodevice_one (PaStreamParameters * StreamParam, int inorout, struct globaldata  * pglobal) {
...
if (maxnumchannel >= 1) {
	...
	if (inorout) {
	// input
		if (pglobal->forcestereo_r) {
			forcestereo=1;
		}; // end if
	} else {
	// output
		if (pglobal->forcestereo_s) {
			forcestereo=1;
		}; // end if
	}; // end else - if
...
void checkaudiodevice_default (struct globaldata * pglobal){
...
void checkaudiodevice_find (struct globaldata * pglobal){
...
for (loop=0; loop<numdevice; loop++) {
	...
	if (strlen(devinfo->name) >= strlen(pglobal->adevice)) {
