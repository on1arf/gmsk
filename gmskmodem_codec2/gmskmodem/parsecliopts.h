...
for (paramloop=1; paramloop <argc; paramloop++) {
	...
	if (strcmp(thisarg,"-rif") == 0) {
		...
		if ((s_global.pttlockfile) || (s_global.pttcsdevice) || (s_global.ptttxdevice) || (s_global.ptt_gpiopin != -1)){
		// PTT already defined
      ...
      if (paramloop+1 < argc) {
         paramloop++;
         s_global.ptt_gpiopin=atoi(argv[paramloop]);

         if (s_global.ptt_gpiopin < 0) {
