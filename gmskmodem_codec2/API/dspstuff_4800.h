...
// Version 20130506: add support for 2400 baud (split into two files: a_dspstuff_2400 and a_dspstuff_4800)
...
float coeffs_table_modulate_4800 [] = {
...
const int firstnonzero_modulate_4800=0; // use complete table
const int lastnonzero_modulate_4800=40; // use complete table
...
float coeffs_table_demodulate_4800 [] = {
		...
		int32_t coeffs_table_demodulate_4800 [] = {
		...
		int32_t coeffs_table_modulate_4800 [] = {
		0, 0, 0, 
		0, 4, 36, 
		281, 1924, 11408, 
...
const int firstnonzero_modulate_4800=4; // element 4 = 5th element in table
const int lastnonzero_modulate_4800=36; // element 36 = 37th element in table
...
const int firstnonzero_modulate_4800=7; // element 7 = 8th element in table
const int lastnonzero_modulate_4800=33; // element 33 = 34th element in table
...
const int firstnonzero_modulate_4800=6; // element 6 = 7th element in table
const int lastnonzero_modulate_4800=34; // element 34 = 35 element in table
...
const int coeffs_size_demodulate_4800=103;
const int buffersize_demodulate_4800=2060; // coeffs_size_demodulate * 20;

const int coeffs_size_modulate_4800=41;
const int buffersize_modulate_4800=820; // coeffs_size_modulate * 20;
...
#if _USEFLOAT == 1
float firfilter_demodulate_4800(struct c2gmsk_session * sessid, float val) {
#else
	#if _INTMATH == 64
	int64_t firfilter_demodulate_4800(struct c2gmsk_session * sessid, int16_t val) {
	#else
	// int32_20 and int32_22
	int32_t firfilter_demodulate_4800(struct c2gmsk_session * sessid, int16_t val) {
	#endif
#endif
...
#if _USEFLOAT == 1
	sessid->dd_filt_buffer=malloc(buffersize_demodulate_4800 * sizeof(float));
#else
	sessid->dd_filt_buffer=malloc(buffersize_demodulate_4800 * sizeof(int16_t));
#endif
...
#if _USEFLOAT == 1
	memset(sessid->dd_filt_buffer,0,coeffs_size_demodulate_4800 * sizeof(float));
#else
	memset(sessid->dd_filt_buffer,0,coeffs_size_demodulate_4800 * sizeof(int16_t));
#endif

	// init vars
	sessid->dd_filt_pointer=coeffs_size_demodulate_4800;
...
ptr1=&sessid->dd_filt_buffer[sessid->dd_filt_pointer-coeffs_size_demodulate_4800];
ptr2=coeffs_table_demodulate_4800;

retval=0;
for (bufferloop=0;bufferloop<coeffs_size_demodulate_4800;bufferloop++) {
#if _USEFLOAT == 1
	retval += (*ptr1++) * (*ptr2++);
#else
# if _INTMATH == 64
...
if (sessid->dd_filt_pointer >= buffersize_demodulate_4800) {
#if _USEFLOAT == 1
	memmove(sessid->dd_filt_buffer,&sessid->dd_filt_buffer[buffersize_demodulate_4800-coeffs_size_demodulate_4800],coeffs_size_demodulate_4800*sizeof(float));
#else
	memmove(sessid->dd_filt_buffer,&sessid->dd_filt_buffer[buffersize_demodulate_4800-coeffs_size_demodulate_4800],coeffs_size_demodulate_4800*sizeof(int16_t));
#endif
	sessid->dd_filt_pointer=coeffs_size_demodulate_4800;
}; // end if
...
}; // end function firfilter_demodulate_4800


///////////////////////////////////////
/// function firfilter for modulation
#if _USEFLOAT == 1
float firfilter_modulate_4800(struct c2gmsk_session * sessid, float val) {
#else
	#if _INTMATH == 64
	int64_t firfilter_modulate_4800(struct c2gmsk_session * sessid, int16_t val) {
	#else
	// int32_10 and int32_12
	int32_t firfilter_modulate_4800(struct c2gmsk_session * sessid, int16_t val) {
	#endif
...
if (sessid->md_filt_init) {
	...
	// allocate memory for buffer
#if _USEFLOAT == 1
	sessid->md_filt_buffer=malloc(buffersize_modulate_4800 * sizeof(float));
#else
	sessid->md_filt_buffer=malloc(buffersize_modulate_4800 * sizeof(int16_t));
#endif
	...
	// init buffer: all zero for first "coeffs_size_demodulate" elements.
#if _USEFLOAT == 1
	memset(sessid->md_filt_buffer,0,coeffs_size_modulate_4800 * sizeof(float));
#else
	memset(sessid->md_filt_buffer,0,coeffs_size_modulate_4800 * sizeof(int16_t));
#endif

	// init vars

	sessid->md_filt_pointer=coeffs_size_modulate_4800;
	p_firstnonzero_modulate=&coeffs_table_modulate_4800[firstnonzero_modulate_4800];
...
// go throu all the elements of the coeffs table
ptr1=&sessid->md_filt_buffer[sessid->md_filt_pointer-coeffs_size_modulate_4800];
ptr2=p_firstnonzero_modulate;

retval=0;
for (bufferloop=firstnonzero_modulate_4800;bufferloop<=lastnonzero_modulate_4800;bufferloop++) {
#if _USEFLOAT == 1
	retval += (*ptr1++) * (*ptr2++);
#else
#if _INTMATH == 64
...
// If pointer has moved past of end of buffer, copy last "coeffs_size_demodulate"
// elements to the beginning of the table and move pointer down
if (sessid->md_filt_pointer >= buffersize_modulate_4800) {
#if _USEFLOAT == 1
	memmove(sessid->md_filt_buffer,&sessid->md_filt_buffer[buffersize_modulate_4800-coeffs_size_modulate_4800],coeffs_size_modulate_4800*sizeof(float));
#else
	memmove(sessid->md_filt_buffer,&sessid->md_filt_buffer[buffersize_modulate_4800-coeffs_size_modulate_4800],coeffs_size_modulate_4800*sizeof(int16_t));
#endif
	sessid->md_filt_pointer=coeffs_size_modulate_4800;
}; // end if
...
///////////////////////////////////////
/// function demodulate
int demodulate_4800 (struct c2gmsk_session * sessid, int16_t audioin) {
...
#if _USEFLOAT == 1
filterret=firfilter_demodulate_4800(sessid, (float)audioin);
#else
	#if _INTMATH == 3210
	// reduce audio to 10 bits (down from 16)
	filterret=firfilter_demodulate_4800(sessid, audioin >> 6);
	#else
	#if _INTMATH == 3212
	// reduce audio to 12 bits (down from 16)
	filterret=firfilter_demodulate_4800(sessid, audioin >> 4);
	#else
	// 64 bit integer math: process audio as 16 bits
	filterret=firfilter_demodulate_4800(sessid, audioin);
	#endif
	#endif
...
sessid->dd_dem_m_pll += PLLINC_4800;
